...
voicebot = Discordrb::Commands::CommandBot.new token: TOKEN, prefix: 'v!'

voicebot.command(:connect) do |event|
	...
	voicebot.voice_connect(channel)
	"Connected to voice channel: #{channel.name}"
end

voicebot.command(:disconnect) do |event|
	event.voice.destroy
end

# A simple command that plays back an mp3 file.
voicebot.command(:play_mp3) do |event|
...
voicebot.command(:play_dca) do |event|
	voice_bot = event.voice
...
msg_history = {}
whitelist = [302463312560390155]

dedup_bot = Discordrb::Bot.new token: TOKEN

dedup_bot.message do |event|
	if (whitelist.include? event.author.id) && (event.message.attachments.empty?)
		if msg_history.keys.include? event.author.id
			msg_history[event.author.id] = [event.message.text, msg_history[event.author.id][0]]
		else
			msg_history[event.author.id] = [event.message.text, ""]
		end

		if msg_history[event.author.id][0] == msg_history[event.author.id][1]
			event.message.delete
		end
	end
end

dedup_bot.run background: true
voicebot.run background: true
